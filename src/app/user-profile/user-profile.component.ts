...
export class UserProfileComponent implements OnInit {
  ...
  //gets the user's profile to view
  getUserProfile(): void {
    this.fetchApiData.getOneUser().subscribe((response: any) => {
      ...
      this.fetchApiData.getAllMovies().subscribe((response: any) => {
        this.favorite_movies = response.filter((m: { _id: any }) => this.user.favorite_movies && this.user.favorite_movies.indexOf(m._id) >= 0);
